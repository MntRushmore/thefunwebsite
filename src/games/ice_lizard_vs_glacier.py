...
class Enemy:
    ...
    def draw_frozen_ice(self, surf, sx, sy):
        ...
        surf.blit(ice_layer, (gx, gy))

    def draw_realistic_details(self, surf, camera_x, camera_y, t, body_color, wing_color, glow_color):
        # Enhanced, higher-detail overlays: scales, specular, wing veins, teeth, nostrils, eye gloss
        sx = self.rect.x - camera_x
        sy = self.rect.y - camera_y
        ...
        # Tuned darker and lighter variants for scale shading
        scale_dark = (
        ...
        # Scales: tiled ellipses with alternating highlights and edges
        rows = max(2, self.rect.height // 6)
        cols = max(3, self.rect.width // 6)

        for ry in range(rows):
            for cx_idx in range(cols):
                px = sx + 6 + cx_idx * (self.rect.width - 12) / max(1, cols - 1)
                py = sy + 8 + ry * (self.rect.height - 16) / max(1, rows - 1)

                # slight jitter for natural look
                jitter = (math.sin((px + py) * 0.07 + t * 2 + self.timer) * 2)
                rect = pygame.Rect(int(px + jitter), int(py + jitter / 2), 8, 5)

                col = scale_light if (rx + cx_idx) % 3 == 0 else scale_dark
                pygame.draw.ellipse(surf, col, rect)
                # tiny rim
                pygame.draw.ellipse(surf, (10, 10, 12), rect, 1)

                # specular dot
                if not frozen:
                    sx_dot = rect.centerx - int(3 * d)
                    sy_dot = rect.centery - 3
                    pygame.draw.circle(surf, (255, 255, 255, 40), (sx_dot, sy_dot), 1)

        # subtle rim lighting along top edge
        top_high = pygame.Surface((self.rect.width, 10), pygame.SRCALPHA)
        for i in range(10):
            alpha = max(0, 28 - i * 2)
            pygame.draw.line(top_high, (255, 255, 255, alpha), (i, 0), (self.rect.width - i, 0))
        surf.blit(top_high, (sx, sy + 2), special_flags=pygame.BLEND_RGBA_ADD)

        # Wing veins
        vein_col = (max(0, wing_color[0] - 40), max(0, wing_color[1] - 40), max(0, wing_color[2] - 40))
        vein_root_left = (bx - 6, by - 4)
        vein_root_right = (bx + 6, by - 4)

        for i in range(3):
            t_off = math.sin(t * 4 + i + self.wing_phase) * 3
            lx = vein_root_left[0] - d * (30 + i * 10)
            ly = vein_root_left[1] - 28 + i * 10 + t_off
            pygame.draw.line(surf, vein_col, vein_root_left, (lx, ly), 1)

            rx = vein_root_right[0] + d * (30 + i * 10)
            ry = vein_root_right[1] - 28 + i * 10 - t_off
            pygame.draw.line(surf, vein_col, vein_root_right, (rx, ry), 1)

        # Teeth rows: small triangles along snout with inner gum shading
        mouth_x = bx + d * 50
        mouth_y = by - 6
        gum_col = (max(0, body_color[0] - 30), max(0, body_color[1] - 30), max(0, body_color[2] - 30))
        ...
        for i in range(4):
            ...
            pygame.draw.polygon(surf, (245, 245, 230), tooth)
            pygame.draw.polygon(surf, (40, 35, 30), tooth, 1)

        # nostrils
        nostril_x = int(hx := (sx + 28 + d * 17) + d * 18)
        nostril_y = int(head_y := (sy + 13) - 4)
        pygame.draw.ellipse(surf, (35, 20, 20), (nostril_x - 2, nostril_y - 2, 4, 3))
        pygame.draw.ellipse(surf, (90, 70, 65), (nostril_x - 3, nostril_y - 1, 6, 3), 1)

        # Eye: layered glossy iris with highlight
        eye_pos = (int(hx - d * 9), int(head_y + 2))
        iris_col = (max(20, body_color[0] - 20), max(20, body_color[1] - 20), max(40, body_color[2] - 30))
        pygame.draw.circle(surf, iris_col, eye_pos, 4)
        pygame.draw.circle(surf, (20, 20, 20), eye_pos, 2)
        pygame.draw.circle(surf, (255, 255, 255), (eye_pos[0] + d, eye_pos[1] - 2), 1)

        # Small glow runes or embers beneath when active
        if not frozen and self.can_attack:
            for i in range(3):
                px = sx + 12 + i * 12
                py = sy + self.rect.height - 12 - int(math.fabs(math.sin(t * 3 + i)) * 6)
                pygame.draw.circle(surf, glow_color, (int(px), int(py)), 3, 1)

        # Subtle ambient occlusion - darken creases
        ao = pygame.Surface((self.rect.width, self.rect.height), pygame.SRCALPHA)
        for i in range(4):
            pygame.draw.ellipse(ao, (0, 0, 0, 12), (i * 4, self.rect.height - 10 - i * 2, self.rect.width - i * 8, 16))
        surf.blit(ao, (sx, sy), special_flags=pygame.BLEND_RGBA_SUB)
